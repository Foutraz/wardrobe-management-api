...
| **MySQL 8.4 via Docker Desktop et Laravel Sail, pas PostgreSQL** | Choix de l'utilisateur pour son environnement local. Supprime le besoin de `sudo` pour la base, et MySQL 8 supporte les contraintes `CHECK`, ce que la conception initiale croyait réservé à PostgreSQL. Le tableau « Pile technique » du README fait foi. |
| Les tests tournent sur MySQL, pas sur SQLite | Sail retire l'override SQLite de `phpunit.xml`. Même moteur qu'en production, donc plus d'écart de comportement sur les contraintes et le JSON. Un override en ligne de commande reste possible pour une exécution rapide hors conteneur. |
...
| La contrainte `CHECK` sur `outfit_items` est ajoutée par une migration conditionnelle | Le Blueprint de Laravel n'expose pas `check()`. La migration l'ajoute en SQL brut sur MySQL, MariaDB et PostgreSQL, et ne fait rien sur SQLite, qui ne peut pas l'ajouter par `ALTER`. Le listener `GuardOutfitItemReference` reste la ceinture, la contrainte est la bretelle. |
...
| Chaque layer enregistre ses propres cascades sur les modèles dont elle dépend | `styling` écoute `Garment::deleting` et `WishlistItem::deleting` pour retirer ses `outfit_items`. Sans cela la clé étrangère bloquait la suppression définitive d'un vêtement figurant dans une tenue — et le cas ne passait que par chance, selon l'ordre d'enregistrement des listeners. Une suppression doit être autonome, jamais dépendante de cet ordre. |
